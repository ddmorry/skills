...
②により fork も vendor もせず上流プラグインを1エントリで載せ、`design-feature` / `build-feature` が実行時に呼ぶ referenced 依存（`to-spec` / `to-tickets` / `tdd` / `code-review` / `grilling` / `domain-modeling`）を同一スコープに供給している。動機と Claude Tag 文脈は [claude-tag-distribution.md](./claude-tag-distribution.md)。
