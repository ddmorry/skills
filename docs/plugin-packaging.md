...
- ③ `soramichi-accounting-plugin` → `source: { github, repo: soramichi-git/soramichi-accounting-plugin, ref: main }`（**外部参照・機密系 private 別リポ**。ADR-0006 のカテゴリ別プラグイン方針で、カード設定など機密情報を含むため実体を別 private リポに置きカタログからは参照だけ載せる。旧 `soramichi-dev/soramichi-kaikei-plugin` の後継。②と違い `sha` ピンは付けず `main` 追随＝自前運用でドリフト検知の対象外）
