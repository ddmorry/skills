...
→ **方針: Claude Tag を前提に `afk-build-feature` を fork し、「Issue ごとに PR を出して人間マージ前提」のスキルを新設する。** 同様に `afk-ship` も Claude Tag 版は「PR 作成で停止（マージしない）」に分岐する。`afk-build-feature` の worktree/AFK ロングランは1スレッド使い捨て sandbox（idle で解放）と相性が悪いので、Issue 単位で PR を出す形に寄せる／routines で回すなど、実挙動を PoC で測って詰める。
