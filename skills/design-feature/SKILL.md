---
name: design-feature
description: 設計段階（grill → to-spec → to-tickets → to-design-doc → afk-ship）を完走させる対話型コンダクター。afk-build-feature（AFK・無人）の対。既存スキルを複製せず順番に運転し、flag 付きスキル（to-spec / to-tickets）は要所でユーザーにスラッシュコマンドを打ってもらい、モデル発動可能なスキル（grilling / to-design-doc / afk-ship）は直接呼ぶ。成果物検出で途中からの再開にも対応。ユーザーが「機能を設計して」「設計段階を回して」「設計パイプラインを回して」「grill から Issue 分解まで進めて」「afk-build-feature（旧 build-feature / run-epic）の準備をして」「Design Doc まで完走して」など、実装前の設計チェーンを通しで進める指示をしたとき必ずこのスキルを使う。
argument-hint: "設計したい機能の説明（途中再開時は PRD/Epic の Issue 番号。省略時は会話中の機能）"
---

# Design Feature — 設計段階を完走するコンダクター

afk-build-feature（実装段階の AFK オーケストレータ）の対になる HITL コンダクター。機能（Feature）1件の設計段階チェーン grill → to-spec → to-tickets → to-design-doc → afk-ship を、既存スキルを複製せずに順番に運転する。持ち物は3つだけ: **順序**・**現在地の検出**・**工程間の引き継ぎ**。実体の作業と人間チェックポイントは各工程のスキル自身が持つ。
...
- **モデル発動可能なスキルは直接呼ぶ**: `grilling` / `domain-modeling` / `grill-yourself-with-docs` / `to-design-doc` / `afk-ship` は Skill ツールで呼ぶ（`grilling`・`domain-modeling` は matt pocock スキル＝別途セットアップ。未導入時のフォールバックは工程1参照）。
...
### 6. afk-ship（Design Doc を作った場合）

`afk-ship` スキルで `docs/design/` の変更を出荷し、origin/main 到達を確認する（= afk-build-feature のプリフライトが通る状態。QA は「非該当: 設計文書のみ」）。

## 完走と締め

終点は「**スライス Issue 起票済み ＋（作った場合）Design Doc が origin/main に到達**」。締めに要約 — 確定したルート・起票した Issue 一覧・Design Doc のパス — を出し、「次は afk-build-feature（`Epic を回して` 等）」と案内して終わる。**afk-build-feature は発動しない**（AFK ロングランの開始は別の意思決定）。
