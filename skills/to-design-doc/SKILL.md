...
Design Doc は**委譲開始前に origin/main に到達している必要がある**（build-feature がプリフライトで確認する）。コミット・マージは通常の出荷フロー（ship）に従う — このスキルでは push しない。ユーザーに「ship してから build-feature」と一言案内して終える。
