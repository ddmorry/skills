...
> **Project ⇄ Project worktree の対応（プレースホルダではなく固定規約）**: Project worktree/ブランチ名は `proj-<slug>` で、`<slug>` は Linear の Project 名から dispatcher が**決定的に**導出する（決定的でないと冪等性が崩れるため、翻訳ではなく機械導出）。優先順位は **(1) Project 名に埋め込んだ明示トークン `[english-slug]`（例「八十二銀行 借入ロールオーバー [82bank-rollover]」→ `proj-82bank-rollover`）→ (2) Project 名全体を ASCII slug 化（英語/混在名。40字上限）→ (3) 純非 ASCII 名は project id 先頭8桁 → `proj-<id8>`（不透明・要 `[slug]` 追記）**。**日本語のみの Project 名では (3) の不透明 id8 になるので、Orca 上で何の Project か分かるよう Project 名末尾に `[english-slug]` を付けることを推奨**（dispatcher は id8 に落ちる Project に警告を出す）。dispatcher は worktree を Linear Project に**ネイティブ紐付けできない**（Orca に `--linear-project` は無い）ため、この決定的な名前で同一性・冪等性を担保する。
