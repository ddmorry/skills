...
# AFK Build Feature — 機能の Issue 群を Coding Agent に委譲して組み上げる
...
3. **Design Doc プリフライト**: Issue 本文が `## Design doc` を参照している場合、`git fetch origin` 後に参照先が origin/main に存在するか確認。無ければ**開始前に停止**し、先に出荷（afk-ship）するよう促す。
...
5. **coding-agent の可用性確認**: agent 一覧に `coding-agent` が無ければ、このスキルの base dir から `agents/coding-agent.md` を `~/.claude/agents/coding-agent.md` へ symlink する（file watching で反映される）。それでも使えない場合のみ `subagent_type: "general-purpose"` + `model: "claude-opus-4-8"` に agent 定義本文を委譲プロンプト冒頭へ貼り込む（このフォールバック経路では `effort` は Agent 呼び出しで指定できないため xhigh を保証できない点に注意）。SendMessage が deferred なら ToolSearch でロードしておく。
...
- Agent ツールで起動: `subagent_type: "coding-agent"`（model / effort 指定不要 — frontmatter の `model: claude-opus-4-8` と `effort: xhigh` が効く。Agent 呼び出しで `model` を渡すと frontmatter より優先され上書きされるので渡さないこと）、`isolation: "worktree"`、`run_in_background: true`。
