...
会社で共有するスキルは、この正本リポジトリから **`soramichi-git/soramichi-plugins`**（GitHub）へ**一方向ミラー**で配布する。ローカル作業コピーは **`code/soramichi/plugins`**（`code/soramichi` は git リポジトリではない素のディレクトリなので、その直下に独立リポジトリとして置ける＝入れ子にならない）。
...
- **配布プラグイン名は `soramichi-dev-plugins`（dev カテゴリ）、marketplace（カタログ）名は `soramichi-plugins`、GitHub リポジトリ名は `soramichi-git/soramichi-plugins`**。命名は `plugins` で統一している（配布単位は「スキル集」ではなくプラグイン＝skills に加え agents / commands / hooks / MCP を束ねる導入単位のため）。install は `soramichi-dev-plugins@soramichi-plugins`、スキルの名前空間は `/soramichi-dev-plugins:<skill>`。今後 finance / accounting / common などカテゴリが増える際は、**カタログを `soramichi-plugins` に集約したままプラグインをカテゴリごとに分割**し（機密系は `source: github` の別リポ外部参照、非機密系は同リポ）、メンバーは必要なプラグインだけ install する（ハイブリッド。判断は ADR-0006）。**実適用済み**: 2026-07-21 に最初のカテゴリ別プラグイン `soramichi-accounting-plugin` を `marketplace.json` に `source: github`（private 別リポ `soramichi-git/soramichi-accounting-plugin`・`ref: main`・sha ピンなし・旧 `soramichi-dev/soramichi-kaikei-plugin` の後継で機密の `card_config.md` を含むため実体は別 private リポ）で追加した（install は `soramichi-accounting-plugin@soramichi-plugins`）。この追加は外部参照だけなので正本の物理再編（`plugins/<cat>/`）は不要で dev は `source: "./"` のまま。プラグイン名前空間はプラグイン経由 install（会社配布・Claude Tag）だけに効き、個別スキル名で張るローカル symlink 運用には影響しない。
...
- **反映は soramichi-git への outward。** `--push` は PR フロー（ブランチ push → PR 作成 → squash マージ → ローカル main 同期）で反映する。※「main 直接 push 禁止」は soramichi-dev 時代の repository rule で、移動後の soramichi-git では現プランでブランチ保護が無効の見込み（下記 org 移動注記）＝直接 push も可能かもしれないが、PR フローは保護の有無に関わらず動くので当面そのまま使う。push / PR 作成 / マージは **ddmorry アカウント**で行う（soramichi-git org を ddmorry が admin として使っており、**別個の soramichi GitHub アカウントは存在しない**）。`code/soramichi/.envrc` の `GH_CONFIG_DIR` 分離は同じ ddmorry の認証情報を graphiq 側と別保存しているだけで、アカウント自体は ddmorry。環境に `GH_TOKEN`（ddmorry）があればそれで通る。commit の author はミラー側 local config で `daisuke_mori@sora-michi.com` に固定（会社リポ表示用のメール分離で、GitHub の pusher は ddmorry になる）。**Claude セッションから `--push` すると、自分が作成した PR の squash マージが自動モード分類器に止められることがある**（その場合は表示される `gh pr merge … --admin` を手動実行）。
- **2026-07-13 org 移動（実施済み）**: 会社ミラーの GitHub 置き場を `soramichi-dev` org から `soramichi-git` org へ **transfer 済み**（`gh api repos/soramichi-dev/soramichi-plugins/transfer -f new_owner=soramichi-git`。両 org は別々に存在。旧 URL は GitHub が自動リダイレクト）。ローカル `code/soramichi/plugins` の remote と運用ドキュメント／配布メタ（publish スクリプト・`plugin.json` / `marketplace.json`・`README.dist.md`）の slug は `soramichi-git/soramichi-plugins` に更新済み。プラグイン名 `soramichi-dev-plugins` は据え置き（`dev`=カテゴリ）。**運用前提の確認結果**: (a) ddmorry は soramichi-git org の active admin＝**確認済み**。(b) main 直 push 禁止ルールは **soramichi-git の現プランでは無効の見込み**（private repo で `repos/.../rules/branches/main` が 403 "Upgrade to Pro" を返す＝ブランチ保護を設定できない）。直接 push できる可能性が高いが、`publish --push` の PR フローは保護有無に関わらず動くのでそのまま使える（直接 push 化＝簡素化は任意）。(c) Claude GitHub App が soramichi-git に導入済みか（Claude Tag 配布用）は**未確認**。
...
- Claude Tag は各自のローカル `~/.claude` を読まず、Anthropic ホストの sandbox（Claude Code on the web と同じエンジン）で動く。**スキル供給の正規経路は「`soramichi-git/soramichi-plugins` を org プラグイン marketplace として admin 登録」**。メンバー側のインストール作業はゼロ。
...
`~/.claude/skills/` には複数の出所のスキルが同居している。**このリポジトリが正本（編集すべき本物）なのは以下の11個だけ**。それ以外は別ソースが正本なので、ここでは編集しない（`~/.claude/skills/` 側で直接いじっても上流に反映されず、再インストールで上書きされる）。どのスキルを触るときも「正本はどこか」を最初に確かめる。
...
| `linear-orca-workspace-setup` | **このリポジトリ** (`skills/linear-orca-workspace-setup/`) | 同上。新しい業務部門の「Linear チーム階層 ⇄ repo ⇄ Orca worktree（Team→Project→Issue）」連携を業務非依存で**初回セットアップ**する（legal-dock の手組みパターンを汎用化）。2026-07 追加（legal-dock の暫定置き場 `.claude/skills/` から移管）。同月に worktree の対応単位を Issue→Project 中心へ変更し、日々の**運用は `linear-orca-dispatch` へ分離**（このスキルは setup 専用）。**個人専用＝`company-skills.txt` に載せず会社リポには出さない**。ランタイム登録は soramichi consumer config (`.claude-config/skills/`) のみ（`risk-scan` と同じく個人グローバル `~/.claude/skills/` には貼らない） |
| `linear-orca-dispatch` | **このリポジトリ** (`skills/linear-orca-dispatch/`) | 同上。`linear-orca-workspace-setup` の対で、**セットアップ済み repo の日々の運用/dispatch**（進行中 Project の worktree 化・大きめ issue のオンデマンド worktree・統合/クローズ・レーン/Project 追加）を担う。セットアップ要否を面接せず dispatcher の存在1行確認だけで運用に入る（「運用のたびにセットアップ要否を評価する無駄」を無くすため 2 本に分割）。2026-07 追加。**個人専用・登録先は `linear-orca-workspace-setup` と同じ** |
...
- **referenced（実行時に名前で Skill 呼び出し）= 内容変化は自己修復するソフト依存。** `tdd` / `code-review` / `grilling` / `domain-modeling` / `to-spec`・`to-tickets`（design-feature）、`research`（risk-scan / risk-deep-dive が一次資料調査を委譲。v1.1.0 で上流に追加）等。ただし**上流のリネーム/削除は参照を壊す**ので存在だけ監視する（例: 2026-07 に上流が `to-prd`→`to-spec`, `to-issues`→`to-tickets` に改名 → design-feature の参照を追随）。
...
- **検知は `scripts/check-vendored-skills.sh`（ローカル/CI 共通）。** 上流を clone し、vendored のサブツリーハッシュ差分（＋ pin→現在の diff）と referenced の消滅を Markdown で報告する（exit 3=ドリフト / 0=一致）。**照合の基準は上流の最新リリースタグ（`vX.Y.Z`）で、main HEAD ではない**（`npx skills@latest` が入れるのは最新リリースなので、release 前の main 変更を誤検知しないため。`vendor-deps.json` の `upstream.trackRef`／`syncedRelease` に記録）。`vX.Y.Z` タグが無い上流なら HEAD にフォールバックする。
...
3. `bash scripts/check-vendored-skills.sh --bump` で vendored の pin を上流の最新リリースタグに更新。
