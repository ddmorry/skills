...
- **配布プラグイン名は `soramichi-dev-plugins`（dev カテゴリ）、marketplace（カタログ）名は `soramichi-plugins`、GitHub リポジトリ名は `soramichi-git/soramichi-plugins`**。命名は `plugins` で統一している（配布単位は「スキル集」ではなくプラグイン＝skills に加え agents / commands / hooks / MCP を束ねる導入単位のため）。install は `soramichi-dev-plugins@soramichi-plugins`、スキルの名前空間は `/soramichi-dev-plugins:<skill>`。今後 finance / accounting / common などカテゴリが増える際は、**カタログを `soramichi-plugins` に集約したままプラグインをカテゴリごとに分割**し（機密系は `source: github` の別リポ外部参照、非機密系は同リポ）、メンバーは必要なプラグインだけ install する（ハイブリッド。判断は ADR-0006）。プラグイン名前空間はプラグイン経由 install（会社配布・Claude Tag）だけに効き、個別スキル名で張るローカル symlink 運用には影響しない。
