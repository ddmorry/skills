...
`~/.claude/skills/` には複数の出所のスキルが同居している。**このリポジトリが正本（編集すべき本物）なのは以下の10個だけ**。それ以外は別ソースが正本なので、ここでは編集しない（`~/.claude/skills/` 側で直接いじっても上流に反映されず、再インストールで上書きされる）。どのスキルを触るときも「正本はどこか」を最初に確かめる。
...
| `linear-orca-workspace-setup` | **このリポジトリ** (`skills/linear-orca-workspace-setup/`) | 同上。新しい業務部門の「Linear チーム階層 ⇄ repo ⇄ Orca の issue 単位 worktree」連携を業務非依存で初回セットアップする（legal-dock の手組みパターンを汎用化）。2026-07 追加（legal-dock の暫定置き場 `.claude/skills/` から移管）。**個人専用＝`company-skills.txt` に載せず会社リポには出さない**。ランタイム登録は soramichi consumer config (`.claude-config/skills/`) のみ（`risk-scan` と同じく個人グローバル `~/.claude/skills/` には貼らない） |
