...
リポジトリ直下には、スキル群が共有する語彙の正本 `CONTEXT.md` と、スキル設計上の決定を記録した `docs/adr/` がある。スキルを書く・直すときはこの2つに従う（用語は CONTEXT.md、委譲アーキテクチャ・受け入れゲート・Design Doc の形・設計段階コンダクターの形は ADR-0001〜0004、Claude Tag 向けの人間マージ fork（build-feature）は ADR-0005）。
